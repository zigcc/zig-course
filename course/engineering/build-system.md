...
对应地，如果要构建动态库，可以继续使用 `b.addLibrary` 并设置 `.linkage = .dynamic`。
...
值得注意的是，目前 zig 已经将 `target query` 和 `resolved target` 完全分开；`Query` 只描述目标，真正传给 `createModule` 的 `.target` 应是 `ResolvedTarget`。
...
zig 的构建系统还允许我们执行一些额外的命令，例如根据 json 生成某些特定的文件（例如 zig 源代码），构建其他的编程语言（不只是 C / C++），如 Golang、Rust、前端项目构建等等！
