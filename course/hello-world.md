...
`main` 的参数 `init: std.process.Init` 是入口函数可选的参数形式，它携带了运行时的初始化信息，其中的 `init.io` 就是我们做 I/O 所需要的 `Io` 实例。
:::
...
我们通过 `std.Io.File.stdout()` 和 `std.Io.File.stderr()` 获取标准输出和标准错误，再基于 `io` 创建各自的 `writer`。这里传入的缓冲区是空的（`&.{}`），意味着不做缓冲——每一次 `print` 都会直接写出去。
...
与上一段代码的区别只在于：这里为 `stdout` 和 `stderr` 各自提供了一块 1024 字节的缓冲区。`writer` 会先把内容写入缓冲区，直到缓冲区满或者调用 `flush` 时才真正输出，从而减少系统调用的次数。

::: warning ⚠️ 注意
使用缓冲区后，**必须记得 `flush`**，否则程序结束时缓冲区里尚未写出的内容就丢失了。
:::
