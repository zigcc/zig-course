...
Zig 中的字符串字面量是一个[单项指针](../advanced_type/pointer#单项指针)，它指向将字符串进行 UTF-8 编码后得到的字节数组。这个字节数组与 C 语言中的字符串类似，都是[以 NUL 字符结尾](../advanced_type/array#哨兵数组)的。因此，字符串字面量既可以隐式转换为 `[]const u8` 切片，也可以隐式转换为以 0 结尾的 `[*:0]const u8` 指针。
