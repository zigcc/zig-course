...
从类型系统的角度看，任何具体的错误集都是 `anyerror` 的**子类型**。因此，任何错误集都可以隐式转换（向上转型）为 `anyerror`，但反之则不然——从 `anyerror` 到具体错误集的转换需要显式进行，此时编译器会增加一个语言级断言（language-level assert），要求该错误确实存在于目标错误集中，否则会触发安全检查错误。
